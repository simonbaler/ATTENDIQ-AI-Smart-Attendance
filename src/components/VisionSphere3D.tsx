...
export const VisionSphere3D: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const r = 1.45 + (Math.random() - 0.5) * 0.2;
      ...
      // Color variation: blue to violet to cyan
      const isCyan = Math.random() > 0.6;
      colors[i * 3] = isCyan ? 0.2 : 0.4;
      colors[i * 3 + 1] = isCyan ? 0.8 : 0.5;
      colors[i * 3 + 2] = 1.0;
    }
    ...
    const ringGeo1 = new THREE.RingGeometry(1.7, 1.72, 64);
    ...
    const ringGeo2 = new THREE.RingGeometry(2.0, 2.02, 64);
